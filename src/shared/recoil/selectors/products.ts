...
import { productsAtom, productAtom } from 'shared/recoil/atoms/products';
...
export const productSelector = selectorFamily({
  ...
  get:
    (id: string) =>
    ({ get }) =>
      get(productsAtom)?.find((product) => +product.id === +id) ||
      get(productAtom),
  set:
    () =>
    ({ set, reset }, data) =>
      data ? set(productAtom, data) : reset(productAtom),
});
